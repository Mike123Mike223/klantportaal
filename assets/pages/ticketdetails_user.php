...
<body>
    <div class="container">
        <div class="header">
            <a href="mijntickets.php" class="btn-back">Terug naar Mijn Tickets</a>
          
        </div>
        <div class="ticket-details">
            <h2>Onderwerp: <?= htmlspecialchars($ticket['onderwerp']) ?></h2>
            ...
            <div class="messages">
                <?php foreach ($messages as $message): ?>
                    <div class="message <?= $message['admin_id'] ? 'admin-message' : 'user-message' ?>">
                        <p><strong><?= htmlspecialchars($message['gebruiker_naam'] ?: 'Admin') ?>:</strong> <?= htmlspecialchars($message['bericht']) ?></p>
                    </div>
                <?php endforeach; ?>
            </div>
